...
#do just the follow pins first
route_special -connect core_pin -layer_change_range { li1(1) rdl(7) } -block_pin_target nearest_target -core_pin_target first_after_row_end -allow_jogging 1 -crossover_via_layer_range { li1(1) rdl(7) } -nets { VPWR VGND } -allow_layer_change 1 -target_via_layer_range { li1(1) rdl(7) }

#manually create full-sized bottom follow-pin
create_shape \
    -layer met1 \
    -shape followpin \
    -path_segment {2.04 2.040} {761.88 2.040} -width 0.49 \
    -net VPWR \
    -status routed

#manually create full-sized top follow-pin
create_shape \
    -layer met1 \
    -shape followpin \
    -path_segment {2.04 757.950} {761.88 757.950} -width 0.49 \
    -net VGND \
    -status routed

# now create the strips and auto-add the vias
...
write_netlist -include_pg -omit_floating_ports -update_tie_connections  \
        -exclude_leaf_cells post_pnr_lvs.vg
